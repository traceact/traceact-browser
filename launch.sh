...
true # (Dropbox xattr guard applies on macOS only)
...
if [ "$NEEDS_EXTENSION" = "yes" ]; then
  printf '%s' "$EXT_DIR" | xclip -selection clipboard 2>/dev/null || wl-copy 2>/dev/null || true
  echo ""
...
sleep 1
xdg-open "http://127.0.0.1:$PORT/demo" 2>/dev/null || true
echo ""
